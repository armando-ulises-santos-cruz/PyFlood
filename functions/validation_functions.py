...
def plot_hwm_comparison(observed_list, predicted_list, labels, colors):
    """
    Plot observed vs predicted HWMs for multiple models on a 1:1 scatter plot.

    Parameters
    ----------
    observed_list : list of ndarray
        List of arrays containing observed HWM elevations.
    predicted_list : list of ndarray
        List of arrays containing predicted HWM elevations.
    labels : list of str
        Labels for each model to use in the legend.
    colors : list of str
        Colors for each model's points and regression line.
    ...
    x_vals = np.linspace(1.5, 6, 100)
    plt.plot(x_vals, x_vals, 'k--', label="1:1 Line")
    plt.plot(x_vals, x_vals + 0.5, 'y--', linewidth=1.5, label="±0.5m Bounds")
    plt.plot(x_vals, x_vals - 0.5, 'y--', linewidth=1.5)

    for observed, predicted, label, color in zip(observed_list, predicted_list, labels, colors):
        model = LinearRegression().fit(observed.reshape(-1, 1), predicted)
        slope = model.coef_[0]
        intercept = model.intercept_
        r2 = r2_score(observed, predicted)
        plt.scatter(observed, predicted, label=f"{label} (R²={r2:.2f})", color=color, alpha=0.7)
        plt.plot(x_vals, slope*x_vals + intercept, color=color)

    plt.xlim(1.5, 6)
    plt.ylim(1.5, 6)
    plt.xticks(np.arange(1.5, 6.1, 0.5))
    plt.yticks(np.arange(1.5, 6.1, 0.5))
    plt.xlabel("Observed High Water Marks [m]")
    plt.ylabel("Predicted Maximum Water Levels [m] (by model)")
    plt.title("Comparison of High Water Marks (HWMs)")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
